...
let funcionarios = [
  { id: 1, nome: "Ana Maria Silva", idade: "24", cargo: "Designer de Moda", departamento: "Criação", salário: "7.000" },
  { id: 2, nome: "Carlos Eduardo Santos", idade: "26",  cargo: "Gerente de Loja", departamento: "Vendas", salário:"8.000" },
  { id: 3, nome: "Mariana Oliveira", idade: "28", cargo: "Estilista", departamento: "Desenvolvimento de Produto", salário:"12.000" },
];
...
app.get('/funcionarios/listar', authenticate, (req, res) => {
  res.json(funcionarios); // Responde com o array de servidores em formato JSON
});

// Rota para obter um servidor por ID (protegida por autenticação)
app.get('/funcionarios/listarFuncionario/:id', authenticate, (req, res) => {
  ...
  const funcionario = funcionarios.find(s => s.id === id); // Encontra o servidor com o ID especificado
  if (funcionario) {
    res.json(funcionario); // Responde com os dados do servidor encontrado
  } else {
    res.status(404).send('Funcionario não encontrado'); // Responde com status 404 se o servidor não for encontrado
  }
});

// Rota para adicionar um novo servidor (protegida por autenticação)
app.post('/funcionarios/inserir', authenticate, (req, res) => {
  const novoFuncionario = req.body; // Obtém o novo servidor do corpo da requisição
  novoFuncionario.id = funcionarios.length ? funcionarios[funcionarios.length - 1].id + 1 : 1; // Define o ID do novo servidor
  funcionarios.push(novoFuncionario); // Adiciona o novo servidor ao array
  res.status(201).json(novoFuncionario); // Responde com status 201 e o novo servidor em formato JSON
});

// Rota para atualizar um servidor existente (protegida por autenticação)
app.put('/funcionarios/atualizar/:id', authenticate, (req, res) => {
  const id = parseInt(req.params.id); // Converte o parâmetro ID para um número inteiro
  console.log(`Recebida requisição PUT para ID: ${id} com dados: `, req.body); // Loga o ID e os dados recebidos
  const index = funcionarios.findIndex(s => s.id === id); // Encontra o índice do servidor com o ID especificado
  if (index !== -1) {
    funcionarios[index] = { ...funcionarios[index], ...req.body }; // Atualiza o servidor com os novos dados
    res.json(funcionarios[index]); // Responde com o servidor atualizado
  } else {
    res.status(404).send('Funcionario não encontrado'); // Responde com status 404 se o servidor não for encontrado
  }
});
...
app.patch('/funcionarios/atualizar/:id', authenticate, (req, res) => {
  const id = parseInt(req.params.id); // Converte o parâmetro ID para um número inteiro
...
app.delete('/funcionarios/deletar/:id', authenticate, (req, res) => {
  const id = parseInt(req.params.id); // Converte o parâmetro ID para um número inteiro
  console.log(`Recebida requisição DELETE para ID: ${id}`); // Loga o ID recebido
  const index = funcionarios.findIndex(s => s.id === id); // Encontra o índice do servidor com o ID especificado
  if (index !== -1) {
    funcionarios.splice(index, 1); // Remove o servidor do array
